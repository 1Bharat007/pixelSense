...
use windows::Win32::Devices::Display::{
    DestroyPhysicalMonitors, GetMonitorBrightness, GetPhysicalMonitorsFromHMONITOR,
        ...
        unsafe {
            ...
            if GetPhysicalMonitorsFromHMONITOR(hmonitor, &mut physical_monitors).is_ok() {
                ...
                Err(PlatformError::NativeApiUnavailable(
                    "GetPhysicalMonitorsFromHMONITOR failed".into(),
                ))
            }
        }
    }
    ...
    pub fn get_brightness(&self, display: &DisplayInfo) -> Result<u8, PlatformError> {
        ...
        unsafe {
            ...
            if let Some(hmonitor) = state.found_hmonitor {
                ...
                let res = GetMonitorBrightness(
                    phys_monitor.hPhysicalMonitor,
                    &mut min,
                    &mut curr,
                    &mut max,
                );
                ...
                if res != 0 {
                    ...
                    return Err(PlatformError::NativeApiUnavailable(
                        "GetMonitorBrightness failed".into(),
                    ));
                }
            }

            Err(PlatformError::NativeApiUnavailable(
                "Target display HMONITOR not found".into(),
            ))
        }
    }
        ...
        unsafe {
            ...
            if let Some(hmonitor) = state.found_hmonitor {
                ...
                if res != 0 {
                    ...
                    return Err(PlatformError::NativeApiUnavailable(
                        "SetMonitorBrightness failed".into(),
                    ));
                }
            }
