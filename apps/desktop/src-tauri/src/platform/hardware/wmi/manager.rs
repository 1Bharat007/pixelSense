...
impl WmiBrightnessManager {
    ...
    fn get_connection(&self) -> Result<WMIConnection, PlatformError> {
        ...
        WMIConnection::with_namespace_path("ROOT\\WMI", com_con).map_err(|e| {
            PlatformError::NativeApiUnavailable(format!("WMI Connection Error: {}", e))
        })
    }
    ...
    pub fn get_brightness(&self) -> Result<u8, PlatformError> {
        ...
        if let Some(monitor) = results.first() {
            ...
            Err(PlatformError::NativeApiUnavailable(
                "No internal monitor found for brightness read".into(),
            ))
        }
    }
    ...
    pub fn set_brightness(&self, level: u8) -> Result<(), PlatformError> {
        ...
        let results: Vec<WmiMonitorBrightnessMethods> = wmi_con.query().map_err(|e| {
            PlatformError::NativeApiUnavailable(format!("Method query failed: {}", e))
        })?;
        ...
        unsafe {
            for res in results {
                ...
                if class_obj
                    .GetMethod(&method_name, 0, &mut in_params_def, std::ptr::null_mut())
                    .is_err()
                {
                    continue;
                }
                ...
                if let Err(e) = in_params.Put(&BSTR::from("Timeout"), 0, &timeout_var, 0) {
                    return Err(PlatformError::NativeApiUnavailable(format!(
                        "Failed to set Timeout parameter (as i32): {}",
                        e
                    )));
                }
                ...
                if let Err(e) = in_params.Put(&BSTR::from("Brightness"), 0, &bright_var, 0) {
                    return Err(PlatformError::NativeApiUnavailable(format!(
                        "Failed to set Brightness parameter (as u8): {}",
                        e
                    )));
                }
                ...
                match svc.ExecMethod(
                    ...
                    Err(e) => {
                        return Err(PlatformError::NativeApiUnavailable(format!(
                            "ExecMethod WmiSetBrightness failed on {}: {}",
                            escaped, e
                        )))
                    }
                }
        ...
        if success {
            ...
            Err(PlatformError::NativeApiUnavailable(
                "No compatible internal display instance found for WMI brightness control.".into(),
            ))
        }
    }
}
