...
impl DuplicationSession {
    ...
    pub fn capture_frame(&mut self) -> Result<RawFrameBuffer, PlatformError> {
        let duplication = self.duplication.as_ref().ok_or_else(|| {
            PlatformError::NativeApiUnavailable("DXGI Duplication not initialized".into())
        })?;
        let device = self.device.as_ref().ok_or_else(|| {
            PlatformError::NativeApiUnavailable("D3D11 Device not initialized".into())
        })?;
        let context = self.context.as_ref().ok_or_else(|| {
            PlatformError::NativeApiUnavailable("D3D11 Context not initialized".into())
        })?;
        ...
        unsafe {
            ...
            if hr.is_err() {
                return Err(PlatformError::NativeApiUnavailable(
                    "AcquireNextFrame failed".into(),
                ));
            }

            let resource = resource.unwrap();
            let texture: ID3D11Texture2D = resource.cast().map_err(|_| {
                PlatformError::NativeApiUnavailable("Cast to ID3D11Texture2D failed".into())
            })?;
            ...
            for y in 0..height {
                ...
                let slice =
                    std::slice::from_raw_parts(data_ptr.add(row_start), (width as usize) * 4);
