...
use crate::platform::facade::{
    BrightnessPlatform, CapturePlatform, DisplayPlatform, PlatformFacade, PowerPlatform,
    SensorPlatform, SessionPlatform, WindowPlatform,
};
...
impl DisplayPlatform for WindowsPlatform {
    fn discover_displays(&self) -> Result<Vec<DisplayInfo>, PlatformError> {
        ...
        unsafe {
            ...
            if !result.as_bool() {
                return Err(PlatformError::NativeApiUnavailable(
...
impl BrightnessPlatform for WindowsPlatform {
    ...
    fn read_hardware_brightness(&self, display: &DisplayInfo) -> Result<u8, PlatformError> {
        ...
        let is_internal =
            name.contains("internal") || name.contains("laptop") || display.is_primary;
        if is_internal {
...
impl CapturePlatform for WindowsPlatform {
    fn acquire_next_frame(
        &self,
        _display_id: &str,
    ) -> Result<crate::screen_analysis::frame::scaler::RawFrameBuffer, PlatformError> {
        Ok(crate::screen_analysis::frame::scaler::RawFrameBuffer::new(
            vec![0, 0, 0, 255],
            1,
            1,
        ))
...
impl SensorPlatform for WindowsPlatform {
    fn read_ambient_light(&self) -> Result<crate::ambient::models::AmbientReading, PlatformError> {
        ...
        use crate::ambient::models::{
            AmbientEnvironment, AmbientQuality, AmbientReading, AmbientSensorType,
        };
    ...
    fn get_active_window_executable(&self) -> Result<String, PlatformError> {
        unsafe {
            ...
            if hwnd.0 == std::ptr::null_mut() {
                return Err(PlatformError::NativeApiUnavailable(
                    "GetForegroundWindow failed".into(),
                ));
            }
            ...
            let process = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, false, process_id)
                .map_err(|e| {
                    PlatformError::NativeApiUnavailable(format!("OpenProcess failed: {}", e))
                })?;
            ...
            if len == 0 {
                return Err(PlatformError::NativeApiUnavailable(
                    "GetProcessImageFileNameW failed".into(),
                ));
            }
...
impl Platform for WindowsPlatform {
    ...
    fn discover_capabilities(
        ...
        Err(PlatformError::NotImplemented(
            "Windows capability placeholder".into(),
        ))
    }
    ...
    fn set_brightness(
        ...
        if name.contains("internal") || name.contains("laptop") || display.is_primary {
            ...
            Err(PlatformError::NotImplemented(
                "DDC brightness control not implemented".into(),
            ))
        }
    }
    ...
    fn get_config_path(&self) -> Result<String, PlatformError> {
        Err(PlatformError::NotImplemented(
            "Windows config path not implemented".into(),
        ))
    }

    fn send_notification(&self) -> Result<(), PlatformError> {
        Err(PlatformError::NotImplemented(
            "Windows notifications not implemented".into(),
        ))
    }
}
